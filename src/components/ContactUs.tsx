  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage(data.error ? "Failed to submit." : "Failed to submit.");
        setStatus("error");
      }
    } catch (error) {
      ...
      <section id="contact" className="py-24 border-t border-white/5 bg-black">
        <div className="container mx-auto px-6 text-center">
          <div className="max-w-2xl mx-auto bg-surface border border-primary/20 p-12 rounded-sm">
            ...
            <h2 className="text-4xl font-bebas tracking-widest text-white mb-4">
              TRANSMISSION RECEIVED
            </h2>
            <p className="text-white/40 uppercase tracking-[0.2em] text-sm">
              You've been added to the frequency. Expect a signal soon.
            </p>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div>
            ...
            <p className="text-white/40 text-lg leading-relaxed mb-10 max-w-lg">
              Join the inner circle for exclusive unreleased frequencies,
              private event coordinates, and direct communication from the
              studio.
            </p>
