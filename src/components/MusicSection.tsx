...
export default function MusicSection({ tracks, allTracks, setTracks, currentPlayingId, setCurrentPlayingId, showToast }: Props) {
    ...
    useEffect(() => {
        ...
        const handleLoadedMetadata = () => {
            const trackIdx = allTracks.findIndex(t => t.src === audio.getAttribute('src'));
            if (trackIdx !== -1) {
    ...
    const shareTrack = (track: Track) => {
        if (navigator.share) {
            navigator.share({ title: `${track.title} by DJ Surreal`, url: window.location.href });
        } else {
            ...
            <div className={`tracks-container ${view === 'grid' ? 'grid-view' : ''}`} id="tracksContainer">
                {tracks.map((track, idx) => (
                    <TrackCard 
                        ...
                        progress={(currentPlayingId === track.id ? (currentTime / (track.duration || 1)) * 100 : 0)}
